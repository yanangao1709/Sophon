...
class Transmission:
    def __init__(self):
        self.requests = QNModel_1.REQUESTSET
        ...
        self.node_cpa = QNModel_1.NODE_CPA
    ...
    def obtain_transmitting_data(self, all_agent_envs, all_agent_action, r, k):
        # 求请求r的第k条候选路径传输量
        transmitting_data = []
        storing_M = self.obtain_storing_info(all_agent_envs)
        for n in range(len(self.routes[r][k])-1):
            node = self.routes[r][k][n] - 1
            if n==0:    # 起始点
                transmitting_data.append(all_agent_action[node][r])
                continue
            if all_agent_action[node][r] >= storing_M[node][r] + transmitting_data[n-1]:
                transmitting_data.append(storing_M[node][r] + transmitting_data[n-1])
            else:
                transmitting_data.append(all_agent_action[node][r])
    ...
    def obtain_transmission_using(self, all_agent_envs, all_agent_action, r, k, n):
        if n+1 not in self.routes[r][k]:
            return 0
        # 节点n是请求r的起点
        if n == self.requests[r][0]:
            using_data = all_agent_action[r][k]
            return using_data
        storing_M = self.obtain_storing_info(all_agent_envs)
        ...
        for nn in range(len(self.routes[r][k]) - 1):
            ...
            if nn==0:
                transmitting_data.append(all_agent_action[node][r])
                continue
            if all_agent_action[node][r] >= storing_M[node][r] + transmitting_data[nn-1]:
                transmitting_data.append(storing_M[node][r] + transmitting_data[nn-1])
            else:
                transmitting_data.append(all_agent_action[node][r])
        if n+1 == self.requests[r][1]:
            using_data = transmitting_data[pos-1]
        else:
        ...
        for r in range(QNConfig.request_pool_len):
            for k in range(QNConfig.candidate_route_num):
                ...
                for n in range(QNConfig.node_num):
                    node_cap_parameters[r][k][n] = self.H_RKN[r][k][n] * (self.obtain_storing_info(all_agent_envs)[r][k] +
    ...
    def judge_is_feasible(self, r, k):
        ...
        for n in range(QNConfig.node_num):
            ...
            for r_determined in range(QNConfig.request_pool_len):
                if self.h_signals[r] > 0:
                    k_determined = self.h_signals[r_determined]
    ...
    def branch_and_bound(self, target_r, route):
        ...
        if len(feasible_routes) <= 1:
            if len(feasible_routes) == 1:   # 该分支上有一条可行路径，为该请求对target_r选中该条路径，重新计算剪枝后的LP的max_obj以选取最好的路径
                self.h_signals[target_r] = feasible_routes.keys()[0]    # 假设target_r被标记，以计算LP的优化目标
            else:
            ...
            if self.h_obj_signals > max_ILP_obj:    # 当前枝的最优LP解，小于记录局部整数最优解，剪枝
                return
            ...
            for r in range(QNConfig.request_pool_len):
                if self.h_signals[r] == -1:
                    flag = False
                    for k in range(QNConfig.candidate_route_num):
                        if new_solution[new_r*QNConfig.request_pool_len + k] == 1:
                            self.h_signals[r] = k
                            flag = True
                            break
                    if not flag:
                        for k in range(QNConfig.candidate_route_num):
                            solution_list.append(utils.LP_solution(new_solution[new_r * QNConfig.candidate_route_num + k], r, self.routes[r][k]))
                    new_r += 1
            ...
            for nb in range(len(next_branches)):
                ...
                self.branch_and_bound(target_r, next_route)
    ...
    def transmit(self, all_agent_envs, all_agent_action):
        ...
        for r in range(QNConfig.request_pool_len):
            ...
            if not flag:
                for k in range(QNConfig.candidate_route_num):
                    solution_list.append(utils.LP_solution(solution[r*QNConfig.candidate_route_num+k], r, self.routes[r][k]))

        if len(solution_list) != 0:
            # 不等于1的LP结果排序
            sorted_solution = sorted(solution_list, key=lambda solution_list: solution_list.solution)
            self.branch_and_bound(sorted_solution[0].r, None)
                ...
                for n in range(QNConfig.node_num):
                    ...
                    for r in range(self.request_num):
                        if y[r] == 0:
                            ...
                            k = y[r] - 1
                            total_r += self.H_RKN[r][k][n] * (self.obtain_storing_info(all_agent_envs)[r][k] +
                            self.obtain_transmission_using(all_agent_envs, all_agent_action, r, k, n))
