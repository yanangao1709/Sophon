...
def run():
    ...
    for ep in range(AgentConfig.episodes_length + 1):
        ...
        if ep % 100 == 0:
            ...
            pd.DataFrame(draw_episode).to_csv('./save_graph/global_rewards_18.csv', index=False)
            pd.DataFrame(agent_local_rewards).to_csv('./save_graph/agent_local_rewards_18.csv', index=False)
            pd.DataFrame(agent_memory_usage).to_csv('./save_graph/agent_memory_usage_18.csv', index=False)
