...
from Comparison.SophonDeploy.Fixed import GlobalEnvFixed, ILPFixed, LocalEnvFixed
...
def SophonFixed(requests, data_volumes, candidate_routes, completed_requests_num,
    ...
    while True:
        ...
        # 全局环境更新
        r_success_counter, current_topology, completed_parts = global_env.update(all_agent_envs, all_agent_action, Y, True)
        if r_success_counter >= completed_requests_num:
            break
        # 局部环境更新
        for agent_id in range(QNConfig.agent_num):
            for p in range(parts):
                next_obs = all_agent_envs[agent_id][p].update(current_topology[p])
                all_agent_obs[agent_id][p] = next_obs
        time_cost += 1
    return time_cost

def SophonFixed_time(requests, data_volumes, candidate_routes, completed_requests_num,
                fidelity_threshold=0.00000001, delay_threshold=150):
    # 初始化各个智能体代理
    agents = {}
    for agent_id in range(QNConfig.agent_num):
...
# 加载已经训练好的分布式代理，由于代理的规模限制，
def SophonFixed_communication_threshold(requests, data_volumes, candidate_routes, completed_requests_num, communication_threshold,
                fidelity_threshold=0.00000001, delay_threshold=150):
    # 初始化各个智能体代理
    agents = {}
    for agent_id in range(QNConfig.agent_num):
    ...
    while True:
        ...
        for agent_id in range(QNConfig.agent_num):
            for p in range(parts):
                ...
                all_agent_obs[agent_id][p] = next_obs
        time_cost += 1
        if time_cost >= communication_threshold:
            if r_success_counter >= completed_requests_num:
