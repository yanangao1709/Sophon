from QNEnv.QNModel_1 import ROUTES, NODE_CPA, LINK_LENS
...
class LocalQN():
    ...
    def reset(self):
        ...
        for r in range(QNConfig.request_pool_len):
            for rt in range(len(ROUTES[r])):
                if self.agent_id in ROUTES[r][rt]:
    ...
    def get_local_reward(self, actions, payoff, Y, completed_R, global_reward, current_topology):
        # 单智能体的个体成本 + 来自全局的奖励
        flag = False
        for r in range(QNConfig.request_pool_len):
            if 1 in Y[r]:
        ...
        for r in range(QNConfig.request_pool_len):
            if completed_R[r]:
                ...
                if 1 in Y[r] and self.agent_id in current_topology.ROUTES[r][Y[r].index(1)]:
                    ...
                    self.reward -= self.M[r] + actions[r]
        if global_reward > 0:
            self.reward += payoff + global_reward
        # else:
        #     self.reward -= payoff
        return self.reward

    def step(self, actions, payoff, global_reward, current_topology, Y, completed_R):
        # 当前智能体的动作，全局给当前智能体的回报
        # current_topology 在全局网络中，更新完了的拓扑信息
        next_obs = self.get_obs(current_topology)
        local_reward = self.get_local_reward(actions, payoff, Y, completed_R, global_reward, current_topology)
        # print(local_reward)
        return next_obs, local_reward
