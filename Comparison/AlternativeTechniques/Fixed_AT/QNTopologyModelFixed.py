...
class QNModel():
    ...
    def update_request_pool_and_topology_info(self, r_state, Fixed, exec = False):
        ...
        self.append_r(len(pop_index), Fixed, exec)
    ...
    def append_r(self, new_r_num, Fixed, exec = False):
        for new_r in range(new_r_num):
            if Fixed:
                # -------固定请求个数实验的请求生成---------------
                if len(self.REQUESTSET) == 1:
                    new_r_id = random.randint(0, len(self.REQUESTSET)-1)
                else:
                    new_r_id = 0
                new_r = self.REQUESTSET[new_r_id]
                self.D_VOLUMN.append(self.D_VOLUMN[new_r_id])
