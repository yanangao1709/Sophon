...
def run():
    ...
    for agent in range(QNConfig.node_num):
        agent_local_rewards[agent] = [0 for sp in range(AgentConfig.episodes_length)]
    # episode的执行，需要所有智能体配合完成
    for ep in range(AgentConfig.episodes_length):
        all_agent_envs = {}
        all_agent_obs = {}
        # 为各分布式智能体建立环境
        for agent_id in range(QNConfig.agent_num):
        ...
        while not done:
            ...
            local_payoff, global_reward, done, current_topology, completed_R, success_data = global_env.step(all_agent_envs, all_agent_action, Y, step)
            if global_reward != 0:
                trans.set_new_topology(current_topology)
            # 分布式节点局部状态更新，智能体的动作与环境交互
            for agent_id in range(QNConfig.agent_num):
                next_obs, local_reward = all_agent_envs[agent_id].step(all_agent_action[agent_id], local_payoff[agent_id], global_reward, current_topology, Y, completed_R)
                # save the sample <s, a, r, s'> to the replay memory
                agents[agent_id].append_sample(all_agent_obs[agent_id], all_agent_action[agent_id], local_reward, next_obs, done)
            ...
            if done:
                ...
                # pylab.plot(episodes, global_rewards, 'b')
                # pylab.savefig("./save_graph/cartpole_dqn.png")
                print("---------------episode:", ep, "-------------score:",
                      score, "---------------------max_objs:", sum_max_ILP_obj, "---------------------suc_data:", suc_data_ep)

                # stop training
                if ep == AgentConfig.episodes_length-1:
                    for agent_id in range(QNConfig.agent_num):
                        torch.save(agents[agent_id].model.state_dict(),
                                   "./save_model/node_agent_" + str(agent_id) + ".pth")
        if ep%100 == 0:
            draw_episode = {"episode": [i for i in range(len(global_rewards))], "episode_rewards": global_rewards, "max_obj": max_ILP_objs, "suc_datas": suc_datas}
