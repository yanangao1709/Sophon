...
def Multi_R_time(requests, data_volumes, candidate_routes):
    new_requests = requests.copy()
    new_node_capacity = QNModel.NODE_CPA.copy()

    start_time = datetime.datetime.now().timestamp()
    allocated_r_rt_data_one_time = run_Mulit_R_com(new_requests, candidate_routes, new_node_capacity)
    end_time = datetime.datetime.now().timestamp()
    one_time = (end_time - start_time)/(len(new_requests)/5)

    return one_time


def Multi_R_com(requests, data_volumes, candidate_routes):
    new_requests = requests.copy()
    new_node_capacity = QNModel.NODE_CPA.copy()
    new_data_volumes = data_volumes.copy()
    allocated_r_rt_data_one_time = run_Mulit_R_com(new_requests, candidate_routes, new_node_capacity)
    accumulated_completed_r_route_allocated = [[0 for rt in range(QNConfig.candidate_route_num)] for r in
                                               range(len(new_requests))]
    ...
    while not flag:
        ...
        for r in range(len(new_requests)):
            accumulated_completed_r_route_allocated[r] = [a + b for a, b in
                                                          zip(accumulated_completed_r_route_allocated[r],
                                                              allocated_r_rt_data_one_time[r])]
        ...
        if not flag:
            ...
            allocated_r_rt_data_one_time = run_Mulit_R_com(new_new_requests, new_candidate_routes, new_node_capacity)
